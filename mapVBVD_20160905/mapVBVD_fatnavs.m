function twix_obj = mapVBVD_fatnavs(filename,varargin)
%
%  mapVBVD_fatnavs - Daniel Gallichan (daniel.gallichan@epfl.ch) -
%  Small modifications to original mapVBVD.m (from Philipp Ehses) to also
%  create output structures containing the data collected as
%  FatNavs/FIDNavs/TrackDOTs.
%
%  Original header:
%
%
...
for s=1:NScans
    ...
    % danielg - FatNav objects
    twix_obj{s}.TrackDOTS  = twix_map_obj(arg,'trackdots',filename,version);
    twix_obj{s}.FIDnav_EXTR  = twix_map_obj(arg,'fidnav_extr',filename,version);
    twix_obj{s}.FIDnav  = twix_map_obj(arg,'fidnav',filename,version);
    twix_obj{s}.FatNav  = twix_map_obj(arg,'fatnav',filename,version);
    twix_obj{s}.FatNav_refscan  = twix_map_obj(arg,'fatnav_refscan',filename,version);
    
    % danielg: (but not for FatNav / TrackDOTS / FIDnav)
    twix_obj{s}.imageWithRefscan = twix_map_obj(arg,'imagewithrefscan',filename,version);
    ...
    %%% danielg - here process the FatNav / FIDnav stuff
    maskSkip = logical(mask.MDH_TRACKDOTS | mask.MDH_FIDNAV_EXTR | mask.MDH_FIDNAV | mask.MDH_FATNAV); % danielg
    
    clear tmpMdh
    isCurrScan = logical(mask.MDH_TRACKDOTS);
    for f = fieldnames( mdh ).'
        tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
    end
    twix_obj{s}.TrackDOTS.readMDH( tmpMdh, filePos(isCurrScan) );
    
    clear tmpMdh
    isCurrScan = logical(mask.MDH_FIDNAV_EXTR);
    for f = fieldnames( mdh ).'
        tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
    end
    twix_obj{s}.FIDnav_EXTR.readMDH( tmpMdh, filePos(isCurrScan) );   
    
    clear tmpMdh
    isCurrScan = logical(mask.MDH_FIDNAV);
    for f = fieldnames( mdh ).'
        tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
    end
    twix_obj{s}.FIDnav.readMDH( tmpMdh, filePos(isCurrScan) );
    
    clear tmpMdh
    isCurrScan = logical(mask.MDH_FATNAV & ~mask.MDH_PATREFSCAN);
    for f = fieldnames( mdh ).'
        tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
    end
    twix_obj{s}.FatNav.readMDH( tmpMdh, filePos(isCurrScan) );
    ...
    twix_obj{s}.FatNav_refscan.readMDH( tmpMdh, filePos(isCurrScan) );
       
    clear tmpMdh
    isCurrScan = logical( ~maskSkip & (mask.MDH_IMASCAN |  (... 
    ...
    twix_obj{s}.imageWithRefscan.readMDH( tmpMdh, filePos(isCurrScan) );
    ...
    if arg.bReadNoiseScan
        clear tmpMdh
        isCurrScan = logical(~maskSkip & mask.MDH_NOISEADJSCAN );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
    ...
    if arg.bReadRefScan
        clear tmpMdh
        isCurrScan =  ~maskSkip & (  ( mask.MDH_PATREFSCAN | mask.MDH_PATREFANDIMASCAN )...
                     & ~( mask.MDH_PHASCOR | mask.MDH_PHASESTABSCAN | ...
                          mask.MDH_REFPHASESTABSCAN | ...
                          mask.MDH_RTFEEDBACK | mask.MDH_HPFEEDBACK));
        for f = fieldnames( mdh ).'
    ...
    if arg.bReadRTfeedback
        clear tmpMdh
        isCurrScan = ~maskSkip & ( mask.MDH_RTFEEDBACK | mask.MDH_HPFEEDBACK ) & ~mask.MDH_VOP;
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
        ...
        clear tmpMdh
        isCurrScan = (~maskSkip & mask.MDH_RTFEEDBACK & mask.MDH_VOP );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
    ...
    if arg.bReadPCScan
        ...
        clear tmpMdh
        isCurrScan = ~maskSkip & mask.MDH_PHASCOR & ( ~mask.MDH_PATREFSCAN | mask.MDH_PATREFANDIMASCAN );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
    ...
    if arg.bReadPhaseStab
        clear tmpMdh
        isCurrScan =  ~maskSkip &  ( mask.MDH_PHASESTABSCAN & ~mask.MDH_REFPHASESTABSCAN ) ...
                      & (~mask.MDH_PATREFSCAN    |  mask.MDH_PATREFANDIMASCAN );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
        ...
        clear tmpMdh
        isCurrScan =  ~maskSkip &  ( mask.MDH_REFPHASESTABSCAN & ~mask.MDH_PHASESTABSCAN ) ...
                      & (~mask.MDH_PATREFSCAN   |   mask.MDH_PATREFANDIMASCAN );
        for f = fieldnames( mdh ).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
    ...
    for scan = { 'image', 'noise', 'phasecor', 'phasestab', ...
                 ...
                 'TrackDOTS','FIDnav_EXTR','FIDnav','FatNav','FatNav_refscan','imageWithRefscan'} % danielg
        f = scan{1};
...
mask.MDH_ONLINE             = min(bitand(evalInfoMask1, 2^3), 1); % danielg
mask.MDH_OFFLINE            = min(bitand(evalInfoMask1, 2^4), 1); % danielg
...
mask.MDH_VOP                = min(bitand(mdh.aulEvalInfoMask(:,2), 2^(53-32)),1); % was 0 in VD %% danielg - looks like there was a bug here so I changed this line to match what works for the FatNav stuff...

mask.MDH_TRACKDOTS =   min(bitand(mdh.aulEvalInfoMask(:,2), 2^(59-32)),1); % danielg
mask.MDH_FIDNAV_EXTR = min(bitand(mdh.aulEvalInfoMask(:,2), 2^(61-32)),1); % danielg
mask.MDH_FIDNAV =      min(bitand(mdh.aulEvalInfoMask(:,2), 2^(62-32)),1); % danielg
mask.MDH_FATNAV =      min(bitand(mdh.aulEvalInfoMask(:,2), 2^(63-32)),1); % danielg
